	...
	: m_reader(reader)
{
	...
	if (offset < 512)
		throw std::runtime_error("io_error: File to small to be a DMG");
	...
	if (!simpleWayOK)
	{
		...
		if (rm1)
		{
			...
			else if (GPTDisk::isGPTDisk(rm1))
			{
				...
				throw std::runtime_error("not_implemented: Unknown partition table type");
...
bool DMGDisk::loadPartitionElements(xmlXPathContextPtr xpathContext, xmlNodeSetPtr nodes)
{
	for (int i = 0; i < nodes->nodeNr; i++)
	{
		...
		if (!xpathObj || !xpathObj->stringval)
			throw std::runtime_error("io_error: Invalid XML data, partition Name key not found");
