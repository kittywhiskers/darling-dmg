...
int DMGDecompressor::readSome(char** ptr)
{
	*ptr = m_buf;
	...
	if (rd <= 0)
		throw std::runtime_error("io_error: DMGDecompressor cannot read from stream");
...
int32_t DMGDecompressor_Zlib::decompress(void* output, int32_t count)
{
	...
	do
	{
		if (m_strm.avail_in == 0)
		{
			...
			if (bytesRead <= 0)
				throw std::runtime_error("io_error: Error reading zlib stream");
			processed(bytesRead);
...
int32_t DMGDecompressor_Bzip2::decompress(void* output, int32_t count)
{
	...
	do
	{
		if (m_strm.avail_in == 0)
		{
			bytesRead = readSome(&input);
			if (bytesRead <= 0)
				throw std::runtime_error("io_error: Error reading bz2 stream");
			processed(bytesRead);
...
{
	if (offset < 0)
		throw std::runtime_error("io_error: offset < 0");
	...
	while ( countLeft > 0 )
	{
		...
		if (nb_input_char_used == 0)
			throw std::runtime_error("io_error: nb_input_char_used == 0");
...
int32_t DMGDecompressor_LZFSE::decompress(void* output, int32_t outputBytes)
{
	...
	if (out_size == 0)
		throw std::runtime_error("io_error: DMGDecompressor_LZFSE failed");
