...
void DMGPartition::adviseOptimalBlock(uint64_t offset, uint64_t& blockStart, uint64_t& blockEnd)
{
	...
	if (itRun == m_sectors.begin())
		throw std::runtime_error("io: Invalid run sector data");
...
int32_t DMGPartition::read(void* buf, int32_t count, uint64_t offset)
{
	...
	while (done < count)
	{
		...
		if (!thistime)
			throw std::runtime_error("io: Unexpected EOF from readRun");
...
int32_t DMGPartition::readRun(void* buf, int32_t runIndex, uint64_t offsetInSector, int32_t count)
{
	...
	switch (runType)
	{
		...
		case RunType::LZFSE:
			...
			throw std::runtime_error("unimplemented: LZFSE is not yet supported");
#endif
		case RunType::Zlib:
		case RunType::Bzip2:
		case RunType::ADC:
		{
			...
			if (dec < count)
				throw std::runtime_error("io: Error decompressing stream");
			return count;
		}
		default:
			return 0;
